...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { font-family: 'Inter', sans-serif; background: ${C.white}; color: ${C.text}; min-width: 100%; overflow-x: hidden; border-left: 3px solid ${C.orange}; border-right: 3px solid ${C.orange}; box-sizing: border-box; }
...
function useSEO(page, postTitle) {
  useEffect(() => {
    const meta = PAGE_META[page] || {
      title: postTitle ? `${postTitle} | Com-Firm` : "Com-Firm",
      desc: "Digitale transformatie met focus, flow en fun."
    };
    ...
    can.href = `https://www.com-firm.com${window.location.pathname}`;
  }, [page, postTitle]);
}
...
function getPageFromPath(pathname) {
  ...
  if (blogPost) return `post-${blogPost[1]}`;
  return path;
}
...
export default function App() {
  ...
  const navigate = (p) => {
    let url;
    if (p === 'home') url = '/';
    else if (p.startsWith('post-')) url = `/blog/${p.replace('post-', '')}`;
    else url = `/${p}`;
  ...
  const postTitle = page.startsWith('post-') ? ALL_POSTS.find(p => p && p.id === page.replace('post-',''))?.title : null;
  useSEO(TOP_PAGES.includes(page) ? page : "blog", postTitle);
